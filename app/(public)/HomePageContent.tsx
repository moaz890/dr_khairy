...
export default function HomePageContent() {
  ...
  return (
    <>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center w-full">
            ...
            <div className="animate-fade-in text-start">
              ...
              <div className="flex flex-wrap gap-4">
                <a
                  href={whatsappUrl("homepage-hero")}
                  target="_blank"
            ...
            <div className="relative animate-fade-in mx-auto w-full max-w-xl lg:max-w-none" style={{ animationDelay: "0.2s" }}>
              ...
              <div className="hidden lg:block absolute -bottom-6 -left-6 bg-white rounded-2xl shadow-2xl p-5 w-56 border border-slate-100 animate-float" style={{ animationDelay: "0.5s" }}>
                ...
                <a
                  href={whatsappUrl("homepage-cta-card")}
                  target="_blank"
                  rel="noopener noreferrer"
      ...
      {/* ── WHY DR. KHAIRY ── */}
      <section id="why-khairy" className="section-white">
        <div className="site-container">
          <div className="text-center mb-12">
            <p className="section-label mb-3">{t.home.whyLabel}</p>
            <h2 className="section-heading mb-4">{t.home.whyHeading}</h2>
            <div className="gold-divider mb-4" />
            <p className="section-sub max-w-2xl mx-auto">{t.home.whySubheading}</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {differentiators.map((item) => (
              <div key={item.id} className="card-premium p-6">
                ...
                <h3 className="text-lg font-bold text-slate-900 mb-2">{item.title[lang]}</h3>
      ...
      <section id="services-preview" className="section-white">
        ...
        <div className="site-container">
          <div className="grid md:grid-cols-3 gap-6 mb-10">
            {homepageServiceGroups.map((slug) => {
              ...
              return (
                <Link key={slug} href={`/services/${slug}`} className="card-premium p-6 group hover:border-cyan-300 transition-colors">
                  ...
                  <p className="text-slate-500 text-sm mb-4 leading-relaxed line-clamp-2">{service.intro[lang]}</p>
                  <span className="text-cyan-900 text-sm font-medium inline-flex items-center gap-1">
                    {t.common.viewAll} <ArrowRight size={14} className="rtl-flip" />
                  </span>
                </Link>
              );
            })}
      ...
      <section id="before-after-preview" className="section-slate">
        <div className="site-container">
          <div className="text-center mb-10">
            <p className="section-label mb-3">{t.beforeAfter.label}</p>
            <h2 className="section-heading mb-4">{t.home.beforeAfterHeading}</h2>
            <div className="gold-divider mb-4" />
          ...
          <div className="text-center">
            <Link href="/before-after" className="btn-outline">
              {t.home.viewBeforeAfter} <ArrowRight size={16} />
            </Link>
          </div>
        </div>
      </section>
      ...
      {/* ── LATEST BLOG ── */}
      <section id="blog-preview" className="section-white">
        <div className="site-container">
          <div className="flex items-end justify-between mb-10">
            <div>
              <p className="section-label mb-2">{t.blog.label}</p>
              <h2 className="text-2xl font-bold text-slate-900">{t.home.latestBlog}</h2>
            </div>
            <Link href="/blog" className="text-cyan-900 text-sm font-medium hover:text-primary flex items-center gap-1 transition-colors">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {blogPosts.slice(0, 3).map((post) => (
              <Link key={post.id} href={`/blog/${post.slug}`} className="card-premium overflow-hidden group">
                ...
                <div className="p-5">
                  <h3 className="font-bold text-slate-900 text-sm leading-snug mb-2 line-clamp-2 group-hover:text-cyan-800 transition-colors">
                    {post.title[lang]}
                  </h3>
                  <p className="text-slate-500 text-xs line-clamp-2">{post.excerpt[lang]}</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      >
        <div className="relative site-container text-center">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            <a
              href={whatsappUrl("homepage-booking-banner")}
              target="_blank"
              rel="noopener noreferrer"
              className="btn-book"
            >
        ...
        <div className="site-container">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {videos.slice(0, 3).map((video) => (
              <Link key={video.id} href={`/videos/${video.slug}`} className="video-card group">
